...
ItemSet.prototype._onDragStartAddItem = function (event) {
  ...
  var start = snap ? snap(time, scale, step, group) : time;
  var end = start;
        ...
        if (props.dragLeft) {
          ...
          if (this.options.rtl) {
            if (itemData.end != undefined) {
              ...
              itemData.end = snap ? snap(end, scale, step, group) : end;
            }
          } else {
            if (itemData.start != undefined) {
              ...
              // TODO: pass a Moment instead of a Date to snap(). (Breaking change)
              itemData.start = snap ? snap(start, scale, step, group) : start;
            }
          }
        }
...
ItemSet.prototype._onAddItem = function (event) {
  ...
  if (event.type == 'drop') {
    ...
    newItemData.start = newItemData.start ? newItemData.start : (snap ? snap(start, scale, step, group) : start);
    ...
    if (newItemData.type == 'range' && !newItemData.end) {
      ...
      newItemData.end = snap ? snap(end, scale, step, group) : end;
    }
  } else {
    newItemData = {
      start: snap ? snap(start, scale, step, group) : start,
      content: 'new item'
    };
